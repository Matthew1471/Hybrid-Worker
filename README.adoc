...
There is no harm in leaving the other keys such as `email` and `validation_key` present in the `credentials.json` as they will be ignored so long as `token` is present. They are removed above for brevity.
